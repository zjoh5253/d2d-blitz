...
import { RepCommissionForm } from "./rep-commission-form";
...
interface RepCommissionsClientProps {
  overrides: OverrideRow[];
  reps: RepOption[];
  carriers: CarrierOption[];
}

export function RepCommissionsClient({
  overrides,
  reps,
  carriers,
}: RepCommissionsClientProps) {
  ...
  async function handleDelete(override: OverrideRow) {
    if (
      !confirm(
        `Delete commission override for "${override.rep.name ?? override.repId}"? This cannot be undone.`
      )
    ) {
      return;
    }
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Custom flat per-install commission for a specific rep. Overrides the
            default plan (tier multiplier bypassed). Most specific wins: product
            &gt; carrier &gt; all.
          </p>
        </div>
      ...
      <DataTable
        ...
        pagination
        pageSize={20}
        emptyMessage="No commission overrides found. Add one to get started."
      />
