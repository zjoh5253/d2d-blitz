...
export function RateSheetForm({
  ...
  // Filter principals by selected level
  const filteredPrincipals = principals.filter((p) =>
    watchedLevel === "OWNER" ? p.role === "MARKET_OWNER" : p.role === "FIELD_MANAGER"
  );

  // When level changes, reset principal
  useEffect(() => {
  ...
  const principalOptions = [
    { value: "", label: "Select principal...", disabled: true },
    ...filteredPrincipals.map((p) => ({ value: p.id, label: p.name ?? p.id })),
  ];
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent onClose={handleClose} className="max-w-md">
        <DialogHeader>
          <DialogTitle>
            {isEdit ? "Edit Rate Sheet" : "Add Rate Sheet"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Flat dollar amount granted to this principal per install.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="text-muted-foreground">
                — Save an owner grant even if it drops the company below its
                minimum margin.
              </span>
            </Label>
          </div>
